...
4. Run `joyride install --user`. This writes hook entries for Claude Code and Codex into the user-level agent configuration, so they apply to every repository on this machine. It also writes cost collection settings, starts a local collector on the loopback interface, and points the global Git `core.hooksPath` at Joyride's Git hooks, which run each repository's own hooks first. The install itself sends nothing. A clone that contains the Joyride workflow joins at the first hook event, and its pushes then publish bounded metadata. Each later push publishes that metadata with a redacted, capped trace of each session, which holds prompts, replies, and tool activity. To keep and publish no trace in this repository, also run `joyride install . --no-traces`.
...
7. Finish with this exact instruction, because hooks load only when a session starts: "Restart this session now." Then stop."""
